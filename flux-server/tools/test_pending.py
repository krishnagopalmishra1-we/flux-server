"""
Targeted smoke test for models not yet tested:
  VIDEO:     wan-t2v-1.3b, wan-t2v-14b, wan-i2v-14b, ltx-video
  MUSIC:     ace-step, audioldm2, stable-audio
  ANIMATION: liveportrait, echomimic

Usage:
    python tools/test_pending.py --server http://104.197.170.202:8080
"""
...
# 1×1 white PNG — tiny valid image for i2v / animation tests
FALLBACK_PNG_B64 = (
...
def test_video(base_url: str, model_name: str, is_i2v: bool, retries: int) -> dict:
    log(f"[video] >> {model_name}")
    for attempt in range(1, retries + 1):
        req = {
            "prompt": f"serene ocean sunset, gentle waves, cinematic — {model_name}",
            "model_name": model_name,
            "resolution": "480p",
            "num_frames": 16,
            "fps": 8,
            "guidance_scale": 5.0,
            "num_inference_steps": 10,   # minimal steps for smoke
            "seed": 42,
        }
        if is_i2v:
        ...
        ok, result = wait_for_job(base_url, job_id, timeout_sec=3600, poll_sec=20)
        if ok:
...
def test_music(base_url: str, model_name: str, retries: int) -> dict:
    ...
    for attempt in range(1, retries + 1):
        req = {
            "prompt": f"calm ambient test track — {model_name}",
            "model_name": model_name,
            "duration_seconds": 6,
            "seed": 42,
        }
...
def main():
    parser = argparse.ArgumentParser(description="Test pending (non-image) models.")
    ...
    log(f"Pending-model smoke test -> {base}  retries={retries}")

    # ── health check ──
    code, health = http_json("GET", f"{base}/health", timeout=60)
    if code != 200:
        raise RuntimeError(f"Health check failed: {code} {health}")
    log(f"Health OK — gpu={health.get('gpu_name','')}  "
        f"vram={health.get('vram_used_gb',0)}/{health.get('vram_total_gb',0)} GB")
    ...
    log("VIDEO MODELS")
    log("=" * 55)
    record(test_video(base, "wan-t2v-1.3b",  is_i2v=False, retries=retries))
    record(test_video(base, "wan-t2v-14b",   is_i2v=False, retries=retries))
