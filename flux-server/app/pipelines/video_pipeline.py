...
def _blend_overlap(tail_frames: list, head_frames: list) -> list:
    """Blend overlapping frames from two chunks using a smooth cosine curve.

    The previous harsh edge-fade (4 frames) caused 'black jumps' because it
    forced a transition to the new chunk before the model's 'cold start' dark
    frames had cleared.

    Fix: Use a cosine alpha across the entire overlap region (N frames).
    This ensures a seamless transition that hides model initialization noise.
    ...
    blended = []
    # Cosine weighting: 1.0 (pure tail) at i=0, 0.0 (pure head) at i=n-1
    # Smooths out the 'black jump' by leaning on established frames from chunk A
    # while chunk B is still 'warming up'.
    for i in range(n):
        a = tail_frames[i]
        b = head_frames[i]
        ...
        # Cosine blend factor: starts at 1.0, ends at 0.0
        # formula: w = 0.5 * (1 + cos(pi * i / (n-1)))
        if n > 1:
            w = 0.5 * (1.0 + np.cos(np.pi * i / (n - 1)))
        else:
            w = 0.0
