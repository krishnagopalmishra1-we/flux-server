#!/bin/bash
# HQ quality test: WAN T2V 14B — 5s (49fr) + 15s (240fr chunked)
# Optimized: 20 steps, 49fr chunks — ~23 min inference (warm) vs 220 min before
...
log "=== HQ TEST: WAN T2V 14B (optimized: 49fr chunks / 20 steps) ==="
...
# ── TEST 1: 5 sec (49 frames @ 16fps, single chunk) ───────────────────────────
# 49fr × 20 steps ≈ 3.3 min inference (vs 55 min at 81fr/50steps)
log "--- TEST 1: WAN T2V 14B 720p / 49fr / 20 steps (3s, single chunk) ---"
RESP=$(curl -s -X POST "$BASE/api/video/generate" \
  -H "Content-Type: application/json" -H "X-API-Key: $API_KEY" \
  -d "{\"prompt\":\"A confident young woman in a vibrant red dress walks through a sunlit city street, crowds of people strolling past shops, ultra-photorealistic 8K HDR, sharp facial details, warm golden hour light, vivid saturated colors, cinematic depth of field, professional cinematography\",\"negative_prompt\":\"$NEG\",\"model_name\":\"wan-t2v-14b\",\"resolution\":\"720p\",\"num_frames\":49,\"fps\":16,\"guidance_scale\":7.5,\"num_inference_steps\":20}" 2>/dev/null || true)
JOB=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin).get('job_id',''))" 2>/dev/null || true)
[ -z "$JOB" ] && { fail "TEST 1 submit failed — $RESP"; } || { pass "TEST 1 submitted: $JOB"; poll_job "$JOB" "WAN T2V 14B 720p/49fr/20s" 3600; }
nvidia-smi --query-gpu=memory.used,utilization.gpu --format=csv,noheader
sleep 5

# ── TEST 2: 15 sec (240 frames @ 16fps, chunked 49fr/20steps) ─────────────────
# 49fr chunks × 20 steps × ~7 chunks ≈ 23 min inference (vs 220 min before)
log "--- TEST 2: WAN T2V 14B 720p / 240fr / 20 steps / 49fr chunks (15 sec) ---"
check_queue
RESP=$(curl -s -X POST "$BASE/api/video/generate" \
  -H "Content-Type: application/json" -H "X-API-Key: $API_KEY" \
...
JOB=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin).get('job_id',''))" 2>/dev/null || true)
[ -z "$JOB" ] && { fail "TEST 2 submit failed — $RESP"; } || { pass "TEST 2 submitted: $JOB"; poll_job "$JOB" "WAN T2V 14B 720p/240fr/20s chunked" 4500; }
nvidia-smi --query-gpu=memory.used,utilization.gpu --format=csv,noheader
